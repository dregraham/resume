...
The **Multi-Cloud IaC Automation Project** demonstrates how **Terraform** and **GitHub Actions** can provision, monitor, and destroy temporary cloud environments on **AWS** (with Azure support planned) while following modern DevOps security practices.
...
4. `.github/workflows/terraform-aws-deploy.yml` responds, executes `terraform apply` against `terraform/main.tf`, waits ≈120 seconds, and then executes `terraform destroy` using remote S3 state and DynamoDB locking.
...
- 🧩 **Terraform-Driven Infrastructure** — declarative IaC for AWS resources (Azure support planned).
- 🔁 **CI/CD Automation** — GitHub Actions workflows validate, plan, apply, and destroy Terraform changes automatically.
- 🔐 **Secure Secrets Management** — GitHub Secrets store IAM credentials with least-privilege access.
- ☁️ **Cloud Logging** — metadata exported to AWS S3 (`dre-multicloud-demo-site`).
...
**Note:** Azure resource support and cross-cloud logging are planned for future releases. Currently, all infrastructure and automation is AWS-only.